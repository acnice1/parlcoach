  ...
  */
  ...
  createApp({
    setup(){
      const state = reactive({
        ...
        vocab: {
        cards: [],        // ← your loaded vocab items [{ fr, en, tags, ... }]
        deck: [],         // working deck (filtered + shuffled copy)
        deckPtr: 0,       // current index
        prefs: {
        randomize: true,          // toggle shuffled order
        withoutReplacement: true, // advance through deck before reshuffle
        },
      },
  ...
  function currentVocabCard() {
    return state.vocab.deck[state.vocab.deckPtr] || null;
  }
  ...
  function nextVocabCard() {
    ...
    if (state.vocab.prefs.withoutReplacement) {
      ...
      state.vocab.deckPtr = Math.floor(rand01() * n);
    }
  }

  function reshuffleVocabDeck() {
  ...
  async function saveDrillPrefs(){
    ...
    await db.drill.put(storable);
      ...
      async function reloadVocabByTag(){
        ...
        state.allCards = rows;
        computeDue();
        // Feed the deck source from SRS cards (front/back → fr/en)
  ...
  function filterVerbsForDrill(list){
    ...
    return out;
      ...
      const api = {
        ...
        // vocab deck
        currentVocabCard, nextVocabCard, buildVocabDeck, reshuffleVocabDeck,
